...
function renderProductRow(category, rowId) {
  ...
  categoryProducts.forEach((product) => {
    rowContainer.innerHTML += `
     <div class="product-deal-card col-lg-3 col-md-4 col-sm-5 col-12 p-0 d-flex flex-column">
  <div class="row pc-top-row px-4 flex-grow-1">
  ...
  // Attach event listeners for "Add to Cart" buttons only once
  const addToCartButtons = document.querySelectorAll(".add-to-cart-btn");
  addToCartButtons.forEach((button) => {
    const productId = parseInt(button.getAttribute("data-id"));
    button.removeEventListener("click", handleAddToCart); // Remove previous event listener if any
    button.addEventListener("click", () => handleAddToCart(productId));
  });
}
